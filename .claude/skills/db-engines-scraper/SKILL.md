...
python scripts/scrape_db_engines.py -o database-info.json

# 2. 补抓失败记录（代理抖动会导致个别内页字段为空；幂等，可多跑几遍）
python scripts/retry_failed.py database-info.json

# 3. 数据清洗 + 规范化 + model/license 枚举预翻译
python scripts/fix_data.py database-info.json

# 4. 翻译 description_cn / developer_cn（见下方"翻译流程"）
python scripts/translate_range.py --start 0 --end 75 --merge batch-0-75.json

# 5. 生成静态分析页（模板在 assets/ 下，单文件离线可用）
python scripts/build_html.py -d database-info.json -t assets/index.template.html -o index.html
```

脚本路径以本 SKILL.md 所在目录为基准；若用户工作目录有同名脚本副本，优先用副本以保持行为一致。
...
python scripts/incremental_update.py --diff

# 2. 执行更新：复用旧数据，只对新增系统抓内页；若有新增会输出 pending-translations.json
python scripts/incremental_update.py --apply
...
python scripts/incremental_update.py --merge-translations pending-translations.json
python scripts/fix_data.py database-info.json
python scripts/build_html.py -d database-info.json -t assets/index.template.html -o index.html
```
...
```bash
# 每个子代理执行（以区间 [0,75) 为例）：
python scripts/translate_range.py --start 0 --end 75            # 输出待翻译清单 JSON
...
- 数据更新后只需重跑步骤 5 即可刷新页面，模板 `assets/index.template.html` 不变；
...
2. 将 `assets/index.template.html` 复制到项目根目录，保持页面模板一致；
